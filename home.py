...
tab1, tab2, tab3, tab4, tab5 = st.tabs(['Поиск вагонов', 'Списки вагонов', '**Характристики**', 'Поиск по предприятиям', 'Список предприятий'])
...
with tab1:
    ...
    if model != '' or model_number !='':
        try:
            ...
            df['Модель'] = df.apply(lambda x: x['name'] + ' мод. ' + x['model'] if x['model_number'] is None else x['name'] + ' мод. ' + x['model'] + ' ' + str(int(x['model_number'])), axis=1)
    ...
    st.caption('Поиск работает только по полному заголовку (можно добавить частичный)')
